...
Wiring<br>
According to the iCE40UP Data sheet on page 34, the maximum current that can be drawn from the I/O pins is 8mA. Using Ohm's Law V=IR without V input being 3.3V, we would need to use a resistor that at least has: (3.3V-0.7V)/0.008A = 325 ohms. The closest upper-bounded one would be the 330 ohms resistor. For the LEDs, the resistor needs to have: (3.3V-2V)/0.02A = 65 ohms (see Lab 1 for details) and the closest choice is the 68 ohms resistor. We choose to use 2N3906 PNP transistors to drive large enough currents toward the anode pins of the dual display with an emitter on 3.3Vcpp and collector to the anode and base to the GPIO pins. <br>
Software<br>
...
![](images\lab2workingactualboard.jpeg)<br>
